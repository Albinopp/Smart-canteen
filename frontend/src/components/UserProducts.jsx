...
export default function UserProducts() {
  ...
  const toggleFeedback = (productId) => {
  ...
  const handleAddProductToCart = async () => {
    try {
      const res = await fetch(
        `http://localhost:8080/addtocart/${selectedProduct.id}/${userId}`,
        {
          method: "POST",
          headers: {
      ...
      <div className="space-y-4">
        {products.length === 0 ? (
          ...
          products.map((product) => (
            <div
              key={product.id}
              className="p-4 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-xl shadow-md"
            >
              ...
              {product.feedback &&
                ...
                openFeedback[product.id] && (
                  <div className="mt-3 bg-white border rounded shadow p-3 max-h-48 overflow-y-auto">
                    {product.feedback.map((fb) => (
                      <div key={fb.id} className="mb-3 border-b pb-2">
                        <p className="font-semibold text-gray-700">
                          ⭐ {fb.rating} — {fb.userName || "Anonymous"}
                        </p>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 w-80">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <input
                ...
                value={quantity}
                min={1}
                max={selectedProduct.quantity * 10}
